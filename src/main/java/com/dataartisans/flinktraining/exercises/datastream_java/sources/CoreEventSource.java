 ...
 *
 */
public class CoreEventSource implements SourceFunction<CoreEvent> {
	 ...
	 */
	public CoreEventSource(String dataFilePath) {
	 ...
	 */
	public CoreEventSource(String dataFilePath, int servingSpeedFactor) {
	 ...
	 * @param servingSpeedFactor The serving speed factor by which the logical serving time is adjusted.
	 */
	public CoreEventSource(String dataFilePath, int maxEventDelaySecs, int servingSpeedFactor) {
	...
	@Override
	public void run(SourceContext<CoreEvent> sourceContext) throws Exception {

		gzipStream = new FileInputStream(dataFilePath);
	...
	private void generateUnorderedStream(SourceContext<CoreEvent> sourceContext) throws Exception {
		...
		CoreEvent event;
		if (reader.ready()) {
			event = CoreEvent.fromReader(reader);
			// extract starting timestamp
			dataStartTime = getEventTime(event);
			// get delayed time
		...
		while (emitSchedule.size() > 0 || reader.ready()) {
			...
			long eventTime = event != null ? getEventTime(event) : -1;
			while(
					event != null && ( // while there is a ride AND
						emitSchedule.isEmpty() || // and no ride in schedule OR
						eventTime < curNextDelayedEventTime + maxDelayMsecs) // not enough rides in schedule
					)
			{
				// insert event into emit schedule
				long delayedEventTime = eventTime + getNormalDelayMsecs(rand);
				emitSchedule.add(new Tuple2<Long, Object>(delayedEventTime, event));
			...
			if(head.f1 instanceof CoreEvent) {
				CoreEvent emitRide = (CoreEvent)head.f1;
				// emit ride
	...
	public long getEventTime(CoreEvent event) {
		return event.getEventTime().getEpochSecond();
	}
